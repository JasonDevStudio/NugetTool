	...
	public partial class FrmNuGetTool : DevExpress.XtraBars.FluentDesignSystem.FluentDesignForm
	{
		...
		private async Task DownloadsAsync()
		{
			try
			{
				...
				// 解析输入的字符串
				var keys = this.txtKeys.EditValue.ToString();
				var packageInfos = keys.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
					.Select(line => line.Trim().Split(' '))
					.Select(parts => new { PackageId = parts[0], Version = parts[1] });

				var _client = this.CreateClient();

				await Parallel.ForEachAsync(packageInfos, async (packageInfo, cancellationToken) =>
				{
					await this.DownloadPackageAsync(packageInfo.PackageId, packageInfo.Version);
					await this.DownloadDependenciesAsync(packageInfo.PackageId, packageInfo.Version);
				});
			}
			catch (Exception ex)
			{
		...
		private void btnOpenFolder_ElementClick(object sender, DevExpress.XtraBars.Navigation.NavElementEventArgs e)
		{
			...
			{
				folderPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "packages");
				if (!Directory.Exists(folderPath))
